...
Возвращает текущее время по МСК, настройки расписания парсера, текущий статус парсера и список маршрутов.
...
Маршрут не ждет завершения обработки. Если запуск принят, сразу возвращает `202 Started`, а сам парсер продолжает работать в фоне.

Если парсер уже работает, ответ будет `409 busy`. Если ручной запуск запрашивают слишком часто, ответ будет `429 rate_limited`.
...
```text
06:00 и 18:00 каждый день по Europe/Moscow
```
